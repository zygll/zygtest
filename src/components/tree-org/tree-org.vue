<template>
  <div ref="zm-tree-org" class="zm-tree-org">
    ...
    <zm-contextmenu
      ...
      :node-edit="nodeEdit"
      :node-copy="nodeCopy"
      :node-add-same-level="nodeAddSameLevel"
      :node-add-notes="nodeAddNotes"
      :node-add-power="nodeaddPower"
      @contextmenu="
        (arg) => {
      ...
      @on-node-delete="
        (node, parentNode) => {
          $emit('on-node-delete', node, parentNode);
        }
      "
    />
  </div>
...
export default {
  ...
  props: {
    ...
    nodeEdit: Function,
    nodeCopy: Function,
    nodeAddSameLevel: Function,
    nodeAddNotes: Function,
    nodeaddPower: Function,
  },
  data() {
    return {
      treeData: {},
  ...
  computed: {
    ...
    dragCancel() {
      return this.draggableOnNode && !this.nodeDraggable
        ? ""
        : ".tree-org-node__content:not(.is-root)>.tree-org-node__inner";
    },
    expandTitle() {
