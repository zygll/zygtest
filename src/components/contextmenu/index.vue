...
export default {
  ...
  methods: {
    ...
    handleAddSameLevel() {
      // 添加同级
      if (this.nodeAddSameLevel) {
        this.nodeAddSameLevel(this.node);
        return;
      }
      const { id, pid, label, expand, children } = this.props;
      ...
      const oldPaNode = this.getNodeById(data, id, node[pid]);
      console.log("oldPaNode", oldPaNode);
      console.log(id, pid, label, expand, children);
      console.log(node);
      const json = {
        [id]: String(new Date().getTime()),
        [pid]: oldPaNode[id],
      ...
      if (Array.isArray(oldPaNode[children])) {
        oldPaNode[children].push(json);
      } else {
        this.$set(oldPaNode, children, [].concat(json));
      }
    },
    handleAddNotes() {
      // 添加备注
